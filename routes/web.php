...
use App\Http\Controllers\Unit\ProposeKpi;
...
Route::middleware(['auth', 'verified', 'account.approved'])->group(function () {
    ...
    Route::middleware('role:president')->group(function () {
        ...
        // Sidebar "Key Result Areas" links — one report page per sub-area code,
        // e.g. /reports/kras/1.1, showing that area's progress reports / exported
        // spreadsheet and the people/units involved. The {code} route parameter
        // includes a dot (e.g. "1.1"), so it's constrained to digits and a dot.
        Route::get('reports/kras/{code}', [ReportController::class, 'kra'])
            ->where('code', '[0-9]+\.[0-9]+')
            ->name('reports.kras.show');

        // Header "Monthly Progress" picker — progress as of a given month/year,
        // e.g. /reports/monthly?year=2026&month=7
    ...
    Route::prefix('my')->name('my.')->middleware('role:staff')->group(function () {
        ...
        Route::post('kpis/bulk', [UnitKpiController::class, 'bulkStore'])->name('kpis.bulk');
        
        // Propose KPI Routes
        Route::get('propose-kpi', [ProposeKpi::class, 'index'])->name('propose-kpi.index');
        Route::post('propose-kpi', [ProposeKpi::class, 'store'])->name('propose-kpi.store'); // <-- Added for form submission
